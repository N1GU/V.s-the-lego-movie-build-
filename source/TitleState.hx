...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		if(!closedState) {
			...
			var http = new haxe.Http("https://raw.githubusercontent.com/N1GU/V.s-the-lego-movie-build-/main/gitVersion.txt");
	...
	function startIntro()
	{
		...
		titleText = new FlxSprite(50).loadGraphic(Paths.image('enter'));
		titleText.setGraphicSize(Std.int(titleText.width * 1.2));
		...
		blackScreen = new FlxSprite(0, 0).loadGraphic(Paths.image('omgcoolbg'));
	...
	override function update(elapsed:Float)
	{
		...
		var pressedEnter:Bool = FlxG.keys.justPressed.ENTER || controls.ACCEPT || FlxG.mouse.pressed;
		...
		if (initialized && !transitioning && skippedIntro)
		{
			if(pressedEnter)
			{
				FlxTween.tween(titleText, { y: titleText.y + 700, x: titleText.x + 700}, 1, { type: FlxTweenType.PINGPONG, ease: FlxEase.quadInOut});
				FlxTween.tween(logoBl, { x: logoBl.x - 700}, 1, { type: FlxTweenType.PINGPONG, ease: FlxEase.quadInOut});
